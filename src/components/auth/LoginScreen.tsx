...
import { ShieldCheck, Smartphone, Lock, UserCheck, AlertCircle, ArrowRight } from 'lucide-react';
...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLoginSuccess }) => {
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    if (result.success && result.user) {
      ...
      setErrorMsg(result.error || 'Login failed. Invalid email/phone or password.');
    }
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
        ...
        <form onSubmit={handleLoginSubmit} className="space-y-4 text-xs">
          ...
          <button
            type="submit"
            className="w-full bg-gradient-to-r from-orange-600 via-amber-600 to-orange-500 hover:from-orange-500 hover:to-amber-500 text-white font-extrabold text-sm py-3.5 px-4 rounded-xl shadow-xl shadow-orange-600/30 flex items-center justify-center gap-2 transition active:scale-95"
          >
