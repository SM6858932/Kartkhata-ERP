...
export const CollectPaymentModal: React.FC<CollectPaymentModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 3. Update Cart Location if checked
      if (updateGpsOnCollect && cart) {
        const updatedLat = cart.currentLat + (Math.random() * 0.001 - 0.0005);
        const updatedLng = cart.currentLng + (Math.random() * 0.001 - 0.0005);
        StorageService.updateCartLocation(cart.id, updatedLat, updatedLng, gpsAddress, currentUser);
      }

      // 4. Open WhatsApp Receipt link
      const waUrl = generateWhatsAppReceiptUrl(
        vendor.phone,
        vendor.fullName,
        { ...summary, month: currentMonth, totalDue: currentTotalDue } as any,
            ...
            <div className="grid grid-cols-3 gap-2">
              {(['cash', 'upi', 'bank_transfer'] as PaymentMode[]).map(mode => (
                <button
                  ...
                  className={`py-2 px-2 text-xs font-bold rounded-xl border capitalize transition ${
                    paymentMode === mode
