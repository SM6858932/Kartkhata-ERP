...
import { scopedCol, scopedDoc } from './firestore';
...
export async function recordPayment(input: RecordPaymentInput): Promise<Payment> {
    const agreementRef = scopedDoc('agreements', input.agreementId);
    const paymentRef = doc(scopedCol('payments'));
    const serialCounterRef = doc(scopedCol('counters'), 'paymentSerial');
...
export async function markReceiptSent(paymentId: string): Promise<void> {
    await runTransaction(db, async tx => {
        tx.update(scopedDoc('payments', paymentId), { receiptSent: true });
    });
}
